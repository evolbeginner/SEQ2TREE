...
mkdir -p $fasta_dir $aln_outdir $trimal_outdir $bsub_etc_outdir $partition_outdir
...
if [ $is_iqtree == true ]; then
	...
	if [ $is_recode == true ]; then
		...
		ruby $pmsf_iqtree -i $recoded_aln --outdir $iqtree_outdir --force --cpu $cpu -m $pmsf_model
	else # normal case
		if [ $is_uni_model == true ]; then
			iqtree -s $combined_aln -m LG+G+I+F -pre $iqtree_outdir/iqtree $bb_str -nt $cpu -redo -wbtl -quiet $tree_add_cmd
			...
			iqtree -s $combined_aln -spp $iqtree_outdir/iqtree.partition -m MFP+MERGE -rcluster 10 -pre $iqtree_outdir/iqtree $bb_str -nt $cpu -redo -wbtl -quiet $tree_add_cmd
		fi
	fi
fi
